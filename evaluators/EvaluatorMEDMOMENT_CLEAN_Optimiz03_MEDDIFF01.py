...
class EvaluatorMEDMOMENT_CLEAN_Optimiz03_MEDDIFF01(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorMEDMOMENT_CLEAN_Optimiz03_MEDDIFF01"
        # self.name = "EvaluatorMEDMOMENT_CLEAN_Optimiz03_MEDDIFF"
    ...
    def evaluarAperturaWAIT(self, results, activeParam, flujo_count):
        ...
        elif results[Constants.IND_PROB_FLOW]==Constants.DIR_PRE_DOWN or results[Constants.IND_PROB_FLOW]==Constants.DIR_DOWN:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                # if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                #     Constants.ACTION_MAX_DIST] >= difference_optimized:
                    ...
                    if results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN or results[
                        ...
                        if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) ==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                            ...
                            print(" INICIO MERCADO WAIT_DOWN_SELL02")

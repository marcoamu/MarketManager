...
class EvaluatorEMA_LONG_03_06_02(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorEMA_LONG_03_06_02"
        # self.name = "EvaluatorEMA_LONG_03_06"
...
#             print(f"ESTAMOS A LA ESPERA DE INDICADORES BUENOS")
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateDownOpen_EMA_WEEK_NEW_FLOW_03(results, activeParam)
        if res:
            if "BUY" in action:
                nada = ""
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                self.printInicioLog(f"SELL_DOWN_BUY_{num}", results, activeParam)
                return
            elif "SELL" in action:
                nada = ""
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        if res:
            if "BUY" in action:
                ...
                nada =""
                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                self.printInicioLog(f"BUY_UP_SELL_{num}", results, activeParam)
                return
    ...
    def evaluarFlujoBUY(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoBUY", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_BUY_WEEK_NEW_DIFF_01(results, activeParam, self.closeAcumValue, self.closeDifference)
    ...
    def evaluarFlujoSELL(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoSELL", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_SELL_EMA_WEEK_NEW_FLOW_04(results, activeParam, self.closeAcumValue, self.closeDifference)

...
class EvaluatorMEDSTDOptimiz04OnlyUp01(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorMEDSTDOptimiz04OnlyUp01"
                ...
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized) or results[
                    ...
                    if self.enableControlOpen:
                        ...
                        if difOpenHours <= 10:
                            ...
                            #no abrir si la diferencia anterior es superior a difference optimized
                            if abs(results[Constants.PREVIOUS_DIST]) <= difference_optimized:
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(" INICIO MERCADO SELLX1")
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        if results[Constants.FLUJO] == Constants.FLUJO_SUBE or results[Constants.FLUJO] == Constants.FLUJO_MANTIENE:
            # if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP or results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UNDEF :
                ...
                if difOpenHours <=1:
                    ...
                    if abs(results[Constants.PREVIOUS_DIST]) <= self.previusOpenDistance:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY

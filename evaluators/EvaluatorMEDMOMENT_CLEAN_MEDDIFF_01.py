...
class EvaluatorMEDMOMENT_CLEAN_MEDDIFF_01(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorMEDMOMENT_CLEAN_MEDDIFF_01"
    ...
    def evaluarAperturaUNDEF(self, results, activeParam, flujo_count):
        ...
        if results[Constants.IND_PROB_FLOW] == Constants.DIR_UP or results[Constants.IND_PROB_FLOW] == Constants.DIR_PRE_UP:
            self.evaluarAperturaUP(results, activeParam, flujo_count)
        if results[Constants.IND_PROB_FLOW] == Constants.DIR_DOWN or results[Constants.IND_PROB_FLOW] == Constants.DIR_PRE_DOWN:
            self.evaluarAperturaDOWN(results, activeParam, flujo_count)
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                ...
                if difOpenHours <= 10:
                    if results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_UP or results[
                        Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN_CHANGE:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        print(f" INICIO MERCADO UP_UP_BUY01 : {results[Constants.ACUMULADO_ABS]}")
                else:

...
class EvaluatorIMA1_ANGLE_03(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorIMA1_ANGLE_03"
        # self.name = "EvaluatorIMA1_ANGLE_02"
    ...
    def cambios(self):
        nada = ""
        # 03-07-2024
        # es igual que angel01 pero tiene distintos cierres
        #para apertura usa ima1
        ...
        if results[Constants.CLOSE_NXT_UP] == 1:
            # if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                ...
                if self.evalNXT_UP_01(results, activeParam, difference_optimized):
                    ...
                    # return
                    nada = ""
        res, action, num = self.evaluateDownOpen_IMA1_02(results, activeParam)
        if res:
            if "SELL" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
    ...
    def evaluarFlujoSELL(self, results, activeParam):
        ...
        self.control_SELL_BLG_LONG_04(results, activeParam, self.closeAcumValue, self.closeDifference, self.accumulate)

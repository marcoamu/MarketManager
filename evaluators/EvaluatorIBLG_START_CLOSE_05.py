...
class EvaluatorIBLG_START_CLOSE_05(EvaluatorBase,AperturaBase,CierreBase):

    #tiene EMA MIN DST

    def __init__(self):
        #se añade cierre mas permisivo
        self.name = "EvaluatorIBLG_START_CLOSE_05"
...
#             print(f"ESTAMOS A LA ESPERA DE INDICADORES BUENOS")
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateDownOpen_start_close_01(results, activeParam)
        if res:
            if "SELL" in action:
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateUpOpen_start_close_01(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
...
#                         print(f"CORRECTION BUY DOWN COUNT")
    ...
    def evaluarFlujoBUY_LONG(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoBUY", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_BUY_START_CLOSE_04(results, activeParam, self.closeAcumValue, self.closeDifference)

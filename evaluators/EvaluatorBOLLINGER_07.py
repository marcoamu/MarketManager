...
class EvaluatorBOLLINGER_07(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorBOLLINGER_06"
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        if results[Constants.CLOSE_NXT_DOWN] == 1:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                ...
                results[Constants.CLOSE_NXT_UP] = 0
                print(
                    f" INICIO MERCADO DOWN_BLG_NXTDOWN_SELL acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                return
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                    ...
                    if results[Constants.IND_BLG_LOWER_DST] < self.blgLowerDist:
                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                        print(
                            f" INICIO MERCADO DOWN_BLG_NXTUP_SELL02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        results[Constants.CLOSE_NXT_DOWN] = 0
                        results[Constants.CLOSE_NXT_UP] = 0
                        return
                    else:
        ...
        if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) == 0) and abs(
            ...
            if results[Constants.IND_BLG] == Constants.IND_BLG_MED_BUY:
                ...
                if results[Constants.ACUMULADO] < 0:
                    if results[Constants.MEDSTDDIFF] < 0:
                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                        print(
                            f" INICIO MERCADO DOWN_DOWN_SELL01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        results[Constants.CLOSE_NXT_DOWN] = 0
                        results[Constants.CLOSE_NXT_UP] = 0
                    ...
                    if results[Constants.MEDSTDDIFF] > 0:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        print(
                            f" INICIO MERCADO DOWN_DOWN_BUY01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        results[Constants.CLOSE_NXT_DOWN] = 0
                        results[Constants.CLOSE_NXT_UP] = 0
                ...
                if results[Constants.IND_BLG_LOWER_DST] < self.blgLowerDist:
                    if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                        ...
                        if results[Constants.ACUMULADO] < 0:
                            if results[Constants.MEDSTDDIFF] < 0:
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(
                                    f" INICIO MERCADO DOWN_DOWN_SELL02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                                results[Constants.CLOSE_NXT_DOWN] = 0
                                results[Constants.CLOSE_NXT_UP] = 0
                            ...
                            if results[Constants.MEDSTDDIFF] > 0:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(
                                    f" INICIO MERCADO DOWN_DOWN_SELLBUY03 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                                results[Constants.CLOSE_NXT_DOWN] = 0
                                results[Constants.CLOSE_NXT_UP] = 0
                    ...
                    if results[Constants.ACUMULADO] > 0:
                        if results[Constants.MEDSTDDIFF] > 0:
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(
                                f" INICIO MERCADO DOWN_DOWN_SELLBUY04 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                            results[Constants.CLOSE_NXT_DOWN] = 0
                            results[Constants.CLOSE_NXT_UP] = 0
        ...
        if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
            if self.enableControlOpen:
                if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                    print(
                        f" INICIO MERCADO PREUP_UP_BUY01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            else:
                if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(
                    ...
                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                    print(
                        f" INICIO MERCADO PREUP_UP_BUY02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                    if results[Constants.INDICATOR_EMA] == Constants.INDICATOR_EMA_BUY:
                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                        print(
                            f" INICIO MERCADO UP_BLG_NXTUP_SELL01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    ...
                    if results[Constants.INDICATOR_EMA] == Constants.INDICATOR_EMA_SELL:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        print(
                            f" INICIO MERCADO UP_BLG_NXTUP_BUY_01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                ...
                if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * multiplicator) or
                    ...
                    if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                        ...
                        results[Constants.CLOSE_NXT_UP] = 0
                        print(
                            f" INICIO MERCADO UP_BLG_NXTDOWN_SELL acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        return
                    elif results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_UP:
                        nada = ""
                        ...
                        results[Constants.CLOSE_NXT_UP] = 0
                        print(
                            f" INICIO MERCADO UP_BLG_NXTDOWN_BUY2 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        return
            ...
            if results[Constants.IND_BLG] == Constants.IND_BLG_MED_SELL:
                ...
                if results[Constants.ACUMULADO] > 0:
                    if results[Constants.MEDSTDDIFF] > 0:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        print(
                            f" INICIO MERCADO UP_UP_BUY01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        results[Constants.CLOSE_NXT_DOWN] = 0
                        results[Constants.CLOSE_NXT_UP] = 0
                else:
                    nada = ""
                    ...
                    if results[Constants.INDICATOR_MED_MOMENT] == Constants.INDICATOR_TM_DOWN:
                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                        print(
                            f" INICIO MERCADO UP_UP_SELL_MOM_01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    ...
                    if results[Constants.ACUMULADO] > 0:
                        ...
                        if results[Constants.CLOSE_NXT_DOWN] == 0:
                            ...
                            if results[Constants.MEDSTDDIFF] > 0:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(
                                    f" INICIO MERCADO UP_UP_BUY02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                                results[Constants.CLOSE_NXT_DOWN] = 0
                                results[Constants.CLOSE_NXT_UP] = 0
                            else:
                                if results[Constants.INDICATOR_MED_MOMENT] == Constants.INDICATOR_TM_UP \
                                    ...
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    print(
                                        f" INICIO MERCADO UP_UP_BUY03 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                                    results[Constants.CLOSE_NXT_DOWN] = 0
                                    results[Constants.CLOSE_NXT_UP] = 0
                        else:
                            if results[Constants.MEDSTDDIFF] < 0:
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(
                                    f" INICIO MERCADO UP_UP_SELLBUY03 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                                results[Constants.CLOSE_NXT_DOWN] = 0
                                results[Constants.CLOSE_NXT_UP] = 0
                            else:
                                nada = ""
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(
                                    f" INICIO MERCADO UP_UP_BYT_NXT_01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                                results[Constants.CLOSE_NXT_DOWN] = 0
                                results[Constants.CLOSE_NXT_UP] = 0
                    else:
                        if results[Constants.ACUMULADO] < 0:
                            if results[Constants.MEDSTDDIFF] < 0:
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(
                                    f" INICIO MERCADO UP_UP_SELL02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
        ...
        if results[Constants.IND_BLG] == Constants.IND_BLG_MED_SELL:
            if results[Constants.ACTION_ACUM] < 0:
                ...
                if results[Constants.ACUMULADO] <= 0:
                    ...
                    if abs(results[Constants.ACUMULADO]) >= self.closeAcumValue:
                        print(
                            f"CERRAMOS MERCADO BUY_CLOSE_ISSELL 01 profit:{results[Constants.ACTION_ACUM]} ")
                        results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                        return

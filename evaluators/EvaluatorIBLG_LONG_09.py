...
class EvaluatorIBLG_LONG_09(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #se añade cierre mas permisivo
        self.name = "EvaluatorIBLG_LONG_09"
...
#             print(f"ESTAMOS A LA ESPERA DE INDICADORES BUENOS")
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
                    ...
                    if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                        if results[Constants.IND_BLG_UPPER_DST_PERCENT] > self.blgDistPercent:
                            if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                                ...
                                self.printInicioLog(f"NXT_UP_BUY_1", results, activeParam)
                            return
                        else:
                            nada=""
        ...
        res, action, num = self.evaluateDownOpen_BLG_01(results, activeParam)
        if res:
            if "SELL" in action:
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateUpOpen_BLG_01(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
    ...
    def evaluarFlujoBUY_LONG(self, results, activeParam):
        ...
        if activeParam.dinamicControl:
            metodo = getattr(self, activeParam.controlBUY, None)
            if callable(metodo):
                metodo(results, activeParam, self.closeAcumValue, self.closeDifference)

        else:
            self.control_BUY_BLG_01(results, activeParam, self.closeAcumValue, self.closeDifference)
    ...
    def evaluarFlujoSELL_LONG(self, results, activeParam):
        ...
        if activeParam.dinamicControl:
            metodo = getattr(self, activeParam.controlSELL, None)
            if callable(metodo):
                metodo(results, activeParam, self.closeAcumValue, self.closeDifference)

        else:
            self.control_SELL_BLG_01(results, activeParam, self.closeAcumValue, self.closeDifference)

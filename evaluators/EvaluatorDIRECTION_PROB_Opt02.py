...
class EvaluatorDIRECTION_PROB_Opt02(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorDIRECTION_PROB_Opt02"
    ...
    def evaluarAperturaCHANGE(self, results, activeParam, flujo_count):
        ...
        if results[Constants.IND_PROB_FLOW]== Constants.DIR_UP or results[Constants.IND_PROB_FLOW]== Constants.DIR_PRE_UP:
                ...
                if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                    ...
                    if results[Constants.PREVIOUS_DIST] <= (self.multiplicatorPREVIUS):
                        if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                            ...
                            print(f" INICIO MERCADO CHANGE_UP_BUY01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
        elif results[Constants.IND_PROB_FLOW]== Constants.DIR_DOWN or results[Constants.IND_PROB_FLOW]== Constants.DIR_PRE_DOWN:
                    ...
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                        ...
                        if results[Constants.PREVIOUS_DIST] <= (self.multiplicatorPREVIUS):
                            if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                                ...
                                print(f" INICIO MERCADO CHANGE_DOWN_SELL01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
        ...
        if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                    if self.enableControlOpen:
                        if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(f" INICIO MERCADO DOWN_DOWN_SELL01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
                    else:
                        ...
                        if difOpenHours <= 20:
                            if self.getProbFlow(results,activeParam)==Constants.DIR_DOWN:
                                if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                    print(f" INICIO MERCADO DOWN_PROB_SELL02 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
                        else:
                            if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                                ...
                                #         difference_optimized * self.multiplicatorPREVIUS):
                                    if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                        print(f" INICIO MERCADO DOWN_DOWN_SELL03 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
            ...
            if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                ...
                if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    print(f" INICIO MERCADO DOWN_REVISAR_SELL01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
            ...
            if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                ...
                #     Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP):
                    if self.enableControlOpen:
                        if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                            print(f" INICIO MERCADO PREDOWN_DOWN_SELL01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
                    else:
                        if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                            print(f" INICIO MERCADO SELL PREDOWN_DOWN_SELL02 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
            elif results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_UP:
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                    Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP):
                    if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                        print(f" INICIO MERCADO SELL PREDOWN_UP_SELL01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
            ...
            if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_UP:
                ...
                #     Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP):
                    ...
                    if self.enableControlOpen:
                        if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(f" INICIO MERCADO PREUP_UP_BUY01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]} meddiff {results[Constants.MEDSTDDIFF]}")
                    else:
                        if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(f" INICIO MERCADO PREUP_UP_BUY02 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
            elif results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                # if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                #     Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP):
                    if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        print(f" INICIO MERCADO PREUP_DOWN_BUY01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                if self.enableControlOpen:
                    if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(f" INICIO MERCADO UP_UP_BUY01 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]} ")
                else:
                    #para abrir sin restricciones ya que es inicio mercado
                    difOpenHours = self.getDifTimeFromOpen(results)
                    if difOpenHours <=10:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        print(f" INICIO MERCADO UP_UP_BUY02 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
                    else:
                        if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                            ...
                            if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                                ...
                                # if results[Constants.RELATIVE]== Constants.INDICATOR_RELATIVE_UP or results[Constants.RELATIVE]== Constants.INDICATOR_RELATIVE_DOWN_CHANGE:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    print(f" INICIO MERCADO UP_UP_BUY03 acum: {results[Constants.ACUMULADO_ABS]} meddiff: {results[Constants.MEDSTDDIFF]}")
        elif results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                    ...
                    results[Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP)):
                        if abs(results[Constants.PREVIOUS_DIST]) <= (
                            ...
                            #     Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN_CHANGE:
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(f" INICIO MERCADO UP_DOWN_BUY01: {results[Constants.ACUMULADO_ABS]}")

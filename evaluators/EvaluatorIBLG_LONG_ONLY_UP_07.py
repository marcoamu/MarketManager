...
class EvaluatorIBLG_LONG_ONLY_UP_07(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #AÑADE FLOW_DIFF
        self.name = "EvaluatorIBLG_LONG_ONLY_UP_07"
        # self.name = "EvaluatorIBLG_LONG_ONLY_UP_06"
        # self.name = "EvaluatorIBLG_LONG_ONLY_UP_05"
        # self.name = "EvaluatorIBLG_LONG_ONLY_UP_02"
        # self.name = "EvaluatorIBLG_LONG_ONLY_UP_01"
    ...
    def evaluate(self, results, activeParam):
        ...
        self.group = activeParam.tele_group
        ...
        self.blgClosePercent = 4
        self.blgCloseAngle = 15
        self.angleUp = activeParam.angleUp
        self.minFlowDiff = activeParam.minflowDiff
                ...
                if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                    ...
                    if results[Constants.IND_BLG_UPPER_DST_PERCENT] > self.blgDistPercent:
                        if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                            ...
                            # nameCount+=1
                            # print(
                            #     f" INICIO MERCADO {name} {results[Constants.DATE].values[0]} \tAcum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} \tMEDSTDDIFF {results[Constants.MEDSTDDIFF]} \tINDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} \tDIRECTION {results[Constants.DIRECTION]} \tINDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]} INDICATOR_MED {results[Constants.INDICATOR_MED]}")
                        ...
                        # results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        # print(
                        #     f" INICIO MERCADO DOWN_BLG_NXTUP_BUY_02 Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        # results[Constants.CLOSE_NXT_DOWN] = 0
                        # results[Constants.CLOSE_NXT_UP] = 0
                        # return
                    ...
                    #     results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                    #     print(
                    #         f" INICIO MERCADO DOWN_BLG_NXTUP_BUY_01 Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    #     results[Constants.CLOSE_NXT_DOWN] = 0
                    #     results[Constants.CLOSE_NXT_UP] = 0
                    #     return
        # INDICADORES
        ...
        if results[Constants.ACUMULADO_ABS] >= (difference_optimized) or results[
            ...
            if self.enableControlOpen:
                if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                    # print(f" INICIO MERCADO DOWN_DOWN_SELL01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]} INDICATOR_MED {results[Constants.INDICATOR_MED]}")
                    nada = ""
                    # results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    # name = "DOWN_SELL_" + nameCount
                    # nameCount += 1
                    # print(
                    #     f" INICIO MERCADO {name} {results[Constants.DATE].values[0]} \tAcum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} \tMEDSTDDIFF {results[Constants.MEDSTDDIFF]} \tINDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} \tDIRECTION {results[Constants.DIRECTION]} \tINDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]} INDICATOR_MED {results[Constants.INDICATOR_MED]}")
                ...
                if results[Constants.IND_BLG_LOWER_DST_PERCENT] >= self.blgDistPercent or results[Constants.IND_BLG_LOWER_DST_PERCENT] ==0:
                    if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                        ...
                        #     results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                        #     print(f" INICIO MERCADO DOWN_DOWN_SELL02 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]} INDICATOR_MED {results[Constants.INDICATOR_MED]}")
                    ...
                    if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP:
                        if results[Constants.IND_BLG_LOWER_DST_PERCENT] < self.blgClosePercent:
                            if results[Constants.FLOW_DIFF] > 0 or abs(
                                    results[Constants.FLOW_DIFF]) <= self.minFlowDiff:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(
                                    f" INICIO MERCADO DOWN_LOWER_DST_01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]}  direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]} INDICATOR_MED {results[Constants.INDICATOR_MED]}")
                            # control para cerrar por si sigue bajando
                    ...
                    #         results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    #         print(
                    #             f" INICIO MERCADO DOWN DOWN_SELLDST_01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]} INDICATOR_MED {results[Constants.INDICATOR_MED]}")
            ...
            if self.enableControlOpen:
                ...
                if results[Constants.IND_BLG_UPPER_DST_PERCENT] >= self.blgDistPercent or results[Constants.IND_BLG_UPPER_DST_PERCENT]==0:
                    if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                        ...
                        #         Constants.ANGLE] == 0:
                        if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP:
                            if results[Constants.FLOW_DIFF] >0 or abs(results[Constants.FLOW_DIFF]) <=self.minFlowDiff:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            ...
                            if abs(results[Constants.WEEK_FLOW_MED]) <5:
                                #esta cambiando
                                if results[Constants.FLUJO]== Constants.FLUJO_SUBE:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    name = "UP_BUY_3"
                    ...
                    if results[Constants.IND_BLG_UPPER_DST_PERCENT] < 0:
                        #esta subiendo
                        if (results[Constants.ANGLE] > 0 and abs(results[Constants.ANGLE]) > self.angleUp) or results[
                                    Constants.ANGLE] == 0:
                            if results[Constants.ANGLE_EMA]>0:
                                if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP:
                                    if results[Constants.FLOW_DIFF] > 0 or abs(
                                            results[Constants.FLOW_DIFF]) <= self.minFlowDiff:
                                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                        print(
                                                    f" INICIO MERCADO UP_UP_BUY_UPPPER_01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]} INDICATOR_MED {results[Constants.INDICATOR_MED]}")

                    else:
                        #vemos si sigue subiendo
                        if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP:
                            if results[Constants.INDICATOR_MED_MOMENT] == Constants.INDICATOR_TM_UP:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(
                                    f" INICIO MERCADO UP_UP_BUY_UPPPER_02 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]} INDICATOR_MED {results[Constants.INDICATOR_MED]}")
                                #cierre rapido por si baja
                                results[Constants.CLOSE_NXT_DOWN] = 1
    ...
    def evaluarFlujoBUY_LONG(self, results, activeParam):
        ...
        unit_diff =0
        print(
            f"DIFFERENCE_OPTIMIZED evaluarFlujoBUY_LONG {difference_optimized} original {activeParam.difference} unit_diffb {unit_diff}")
        ...
        if results[Constants.CLOSE_NXT_DOWN] == 1:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                ...
                #
                print(
                        f" CERRAMOS MERCADO CLOSE_NXT:DOWN Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")

                print(f"EVALUO CLOSE_NXT_DOWN")
                results[Constants.CLOSE_NXT_DOWN] = 0
                results[Constants.CLOSE_NXT_UP] = 0
        ...
        if results[Constants.IND_BLG]== Constants.IND_BLG_MED_SELL:
            if results[Constants.ACTION_COUNT]>5:
                if results[Constants.ACTION_MAX_DIST] > self.closeAcumValue:
                    print(
        ...
        if results[Constants.ACTION_COUNT] > 5:
            if results[Constants.ACTION_ACUM] <= 0:
                ##estamos perdiendo
                print(
                    f"CERRAMOS MERCADO BUY action_5times1: {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_CLOSE_ISSELL 01 profit:{results[Constants.ACTION_ACUM]} ")
                results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                return
            else:
                #tenemos ganancia
                #cerramos si no se gano mucho
                if results[Constants.ACTION_ACUM] <=self.closeAcumValue:
                    print(
                        f"CERRAMOS MERCADO BUY action_5times2: {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_CLOSE_ISSELL 01 profit:{results[Constants.ACTION_ACUM]} ")
                    results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                    return
        #CERRAMOS SI SALE DEL TOP BLG
        if results[Constants.IND_BLG_UPPER_DST_PERCENT]<0:
            #HA pASADO EL TOP HAY GANANCIAS
            if abs(results[Constants.IND_BLG_UPPER_DST_PERCENT])>2:
                print(
        ...
        # CERRAMOS SI SE APROXIMA A TOP Y TIENE UN ANGULO NO DE SUBIDA
        if abs(results[Constants.IND_BLG_UPPER_DST_PERCENT]) < self.blgClosePercent:
            # si esta bajando o el angulo es
            if results[Constants.ANGLEm21] < self.blgCloseAngle:
                print(
                    f"CERRAMOS MERCADO {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_MAXIMO_ANGLE_01 {results[Constants.ACUMULADO]} profit:{self.closeAcumValue}")
                results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                return
    ...
    def evaluarFlujoSELL_LONG(self, results, activeParam):
        ...
        unit_diff = 0
        print(
            f"DIFFERENCE_OPTIMIZED evaluarFlujoSELL_LONG {difference_optimized} original {activeParam.difference} unit_diffb {unit_diff}")
        ...
        if results[Constants.ACTION_ACUM] > 0:
            if abs(results[Constants.ACTION_ACUM]) >= (self.closeAcumValue):
                print(
                    f"CERRAMOS MERCADO {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_ACTION_DIST02 {results[Constants.ACUMULADO]} profit:{self.closeAcumValue}")
                results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                return
        ...
        if results[Constants.IND_BLG_LOWER_DST_PERCENT] < 0:
            ...
            if abs(results[Constants.IND_BLG_LOWER_DST_PERCENT]) > 2:
                print(
        ...
        if results[Constants.CLOSE_NXT_UP] == 1:
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                    ...
                    print(
                        f" CERRAMOS MERCADO SELL_UP_BLG_01 Acum: {results[Constants.ACUMULADO_ABS]} FLOW_DIFF: {results[Constants.FLOW_DIFF]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")

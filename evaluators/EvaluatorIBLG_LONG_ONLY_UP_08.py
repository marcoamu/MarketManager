...
class EvaluatorIBLG_LONG_ONLY_UP_08(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #AÑADE FLOW_DIFF
        self.name = "EvaluatorIBLG_LONG_ONLY_UP_08"
    ...
    def evaluate(self, results, activeParam):
        ...
        self.closeAcumValueShort = activeParam.bollingerClose / 2
...
#                         print(f"CORRECTION BUY DOWN COUNT")
    ...
    def evaluarFlujoBUY_LONG(self, results, activeParam):
        ...
        unit_diff =0
#         print(
            f"DIFFERENCE_OPTIMIZED  evaluarFlujoBUY {difference_optimized} original {activeParam.difference} closeDifference: {self.closeDifference} close ACUM: {self.closeAcumValue} close acum sot: {self.closeAcumValueShort} unit_diffb {unit_diff}")
...
#                 print(
        ...
        if results[Constants.ACUMULADO] < 0:
            if results[Constants.ACTION_ACUM] >= 0:
                ##estamos ganando
                if abs(results[Constants.ACTION_ACUM]) >= (self.closeAcumValue):
                    if abs(results[Constants.ACUMULADO]) >= self.closeAcumValueShort:
#                         print(
                            f"CERRAMOS MERCADO BUY actioacum01: {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_CLOSE_ISSELL 01 profit:{results[Constants.ACTION_ACUM]} ")
                        results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                        return
            else:
                # tenemos perdidas
                if results[Constants.ACTION_COUNT] >= 2:
                    if abs(results[Constants.ACTION_ACUM]) >= (self.closeDifference):
#                         print(
                            f"CERRAMOS MERCADO {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_ACTION_PERDIDAS01 {results[Constants.ACUMULADO]} profit:{self.closeAcumValue}")
                        results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                        return

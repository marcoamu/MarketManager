...
class EvaluatorDistance01(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorDistance01"
                ...
                if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                    # if results[Constants.DIRECTION] == Constants.DIR_UP:
                        if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[Constants.ACTION_MIN_DIST]>= difference_optimized:
                            #cCONTROL NO ENTRAR SI YA EL VALOR ES MUY ALTO
                            if results[Constants.ACUMULADO_ABS] <= float(activeParam.difference*2):
...
#                                 print(f"INICIO MERCADO BUY1 cancelado")
                            # Constants.prepararYenviarMensaje(results, activeParam, data, True)

...
class EvaluatorEMA_LONG_03_05(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorEMA_LONG_03_05"
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateDownOpen_EMA_WEEK_01(results, activeParam)
        if res:
            if "BUY" in action:
                nada= ""
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateUpOpen_EMA_WEEK_03(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
    ...
    def evaluarFlujoBUY(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoBUY", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_BUY_WEEK_04(results, activeParam, self.closeAcumValue, self.closeDifference)
    ...
    def evaluarFlujoSELL(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoSELL", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_SELL_EMA_WEEK_05(results, activeParam, self.closeAcumValue, self.closeDifference)

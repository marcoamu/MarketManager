...
class EvaluatorRSI_ONLYUP_01(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorRSI_ONLYUP_01"
    ...
    def evaluate(self, results, activeParam):
        ...
        if results[Constants.CURRENT_ACTION] == Constants.ACTION_WAIT or results[
            ...
            # self.evaluarApertura(results, activeParam)
                self.evaluarAperturaRSI(results, activeParam)
            else:
    ...
    def evaluarAperturaRSI(self, results, activeParam):
        #
        # valores
        flujo_count = results[Constants.FLUJO_COUNT]

        if ((results[Constants.RSI] >30 and results[Constants.RSI] <42) or (results[Constants.RSI] >50 and results[Constants.RSI] <60))  and results[Constants.RSI_DIFF] > 0:
            ...
            #     self.evaluarAperturaDOWN(results, activeParam, flujo_count)
        elif (results[Constants.RSI] >70 and results[Constants.RSI] <80) and results[Constants.RSI_DIFF] > 0:
            if results[Constants.RSI_SLOPE] > 0:
                self.evaluarAperturaUP(results, activeParam, flujo_count)
        elif (results[Constants.RSI] >20 and results[Constants.RSI] <30) and results[Constants.RSI_DIFF] < 0:
            nada = ""
            # if abs(results[Constants.RSI_SLOPE]) < 0:
            #     self.evaluarAperturaDOWN(results, activeParam, flujo_count)
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateDownOpen_RSI_01(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateUpOpen_RSI_01(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
    ...
    def evaluarFlujoBUY(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoBUY", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_BUY_RSI_01(results, activeParam, self.closeAcumValue, self.closeDifference)
    ...
    def evaluarFlujoSELL(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoSELL", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_SELL_RSI_01(results, activeParam, self.closeAcumValue, self.closeDifference)

...
class EvaluatorIMA1_CLEAN_BLG_EXTREMES_01(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #se añaden sell en only up
        #se añade cierre mas permisivo
        self.name = "EvaluatorIMA1_CLEAN_BLG_EXTREMES_01"
        ...
        if results[Constants.CLOSE_NXT_DOWN] == 1:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                ...
                #                 difference_optimized )):
                    #reconocer cuando sigue en racha
                    # if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                results[Constants.CLOSE_NXT_DOWN] = 0
                results[Constants.CLOSE_NXT_UP] = 0
    ...
    def evaluarFlujoBUY(self, results, activeParam):
        ...
        if results[Constants.IND_BLG_UPPER_DST_PERCENT] <0 or results[Constants.IND_BLG_UPPER_DST_PERCENT] < 5:
            #si hay algo desde que abrio no cerrar inmediatamente
            if results[Constants.ACTION_COUNT]>2:
                ...
                #dejamos close NXT UP para abrir por si estamos subiendo
        ...
        # cerramos lo antes posible si tiene perdidas
        if results[Constants.ACTION_ACUM] < 0:
            # es negativo
            # vericicar si bajo mas de lo esperado
            if abs(results[Constants.ACTION_ACUM]) >= self.closeDifference:
                print(
                    f"CERRAMOS MERCADO BUY action_acum: {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_CLOSE_ACUM02 profit:{results[Constants.ACTION_ACUM]} ")
                results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                return
        ...
        if results[Constants.ACTION_ACUM] < 0:
            ...
            if abs(results[Constants.ACTION_MAX_DIST]) >= self.closeAcumValue:
                print(
                    f"CERRAMOS MERCADO BUY action_acum: {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_CLOSE_ACUM03 profit:{results[Constants.ACTION_ACUM]} ")
                results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                return

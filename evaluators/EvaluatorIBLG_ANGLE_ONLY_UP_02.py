...
class EvaluatorIBLG_ANGLE_ONLY_UP_02(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #se añade cierre mas permisivo
        self.name = "EvaluatorIBLG_ANGLE_ONLY_UP_02"
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                ...
                if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                    ...
                    if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                        ...
                        ##SOLO ABRIR SI esta cerca de un cambio de sentido por week bajo
                        # if results[Constants.WEEK_DIR_BOT_DST] < self.MIN_WEEK_BOT_PERCENT:
...
#                     print(
                ...
                #         if abs(results[Constants.ANGLE_IMA1]) > self.minAngleIma1:
                #             if abs(results[Constants.ACTION_ACUM]) > self.closeDifference:
                #
                #                 print(
                #                     f"CERRAMOS MERCADO BUY action_nxt_up01: {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_CLOSE_ISSELL 01 profit:{results[Constants.ACTION_ACUM]} ")
                #                 results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                #                 return

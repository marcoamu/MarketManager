...
class EvaluatorDistMarketTendence11(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorDistMarketTendence11"
    ...
    def evaluate(self, results, activeParam):
        ...
        multiplicador = 2
        multiplicadorUP = 2
        #para cerrar en perdidas subida bajada
        multiplicadorLose = 2
        ...
        if results[Constants.CURRENT_ACTION] == Constants.ACTION_WAIT or results[Constants.CURRENT_ACTION] == Constants.ACTION_CLOSE:
            ...
            if results[Constants.ACTION_DISTANCE] <= 0:
                if abs(results[Constants.ACTION_DISTANCE]) >= (activeParam.difference /multiplicadorLose):

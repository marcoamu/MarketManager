...
class Evaluator02(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "Evaluator02"
    ...
    def evaluate(self, results, activeParam):
        ...
        if results[Constants.CURRENT_ACTION] == Constants.ACTION_WAIT or results[Constants.CURRENT_ACTION] == Constants.ACTION_CLOSE:
                    ...
                    if results[Constants.INDICATOR_TENDENCE] == Constants.INDICATOR_T_UP:
                        ...
                        if results[Constants.INDICATOR_TENDENCE] == Constants.INDICATOR_T_UP:
                            if results[Constants.ACTION_MIN_DIST] >= difference_optimized:
                                ...
                                #self.prepararYenviarMensaje(results, active, data, True)
            # elif results[self.IMA_NEW] == self.IMA_NEW_SELL:
            #     difference_optimized = active.difference
            #     unit_diff = 0
            #     if results[self.INDICATOR] == self.INDICATOR_SELL:
            #         if results[self.INDICATOR_TENDENCE] == self.INDICATOR_T_WAIT:
            #             difference_optimized = active.difference
            #         elif results[self.INDICATOR_TENDENCE] == self.INDICATOR_T_UP:
            #             if results[self.MARKET_TENDENCE] == self.MARKET_TENDENCE_DOWN:
            #                 if flujo_count > 2:
            #                     unit_diff = active.unit * (flujo_count) * 1.3
            #                     unit_diff = unit_diff * -1
            #                 else:
            #                     difference_optimized = active.difference / 2
            #             else:
            #                 difference_optimized = active.difference + (active.difference / 3)
            #         else:
            #             if flujo_count > 2:
            #                 unit_diff = active.unit * (flujo_count) * 1.3
            #                 unit_diff = unit_diff * -1
            #             else:
            #                 difference_optimized = active.difference - (active.difference / 2)
            #     elif results[self.INDICATOR] == self.INDICATOR_SELLX:
            #         if results[self.MARKET_TENDENCE] == self.MARKET_TENDENCE_DOWN:
            #             if flujo_count > 2:
            #                 unit_diff = active.unit * (flujo_count) * 1.3
            #                 unit_diff = unit_diff * -1
            ...
            #     print(
            #         f"DIFFERENCE_OPTIMIZED {difference_optimized} original {active.difference} unit_diffb {unit_diff}")
            #
            #     if results[self.FLUJO] == self.FLUJO_BAJA:
            #         if results[self.MARKET_TENDENCE] == self.MARKET_TENDENCE_DOWN:
            #             if results[self.ACUMULADO_ABS] >= difference_optimized or results[self.ACTION_MAX_DIST] >= difference_optimized:
            #                 results[self.CURRENT_ACTION] = self.ACTION_SELL
            #                 alert = True
            #                 isSell = True
            #                 self.currentTupla['START'] = results[self.VALUE]
            #                 print(" INICIO MERCADO SELLX1")
            #                 self.prepararYenviarMensaje(results, active, data, True)
            #         elif results[self.INDICATOR_TENDENCE] == self.INDICATOR_T_WAIT:
            #             if results[self.ACTION_MAX_DIST] >= difference_optimized:
            #                 results[self.CURRENT_ACTION] = self.ACTION_SELL
            #                 alert = True
            #                 isSell = True
            #                 self.currentTupla['START'] = results[self.VALUE]
            ...
            #             # if results[self.IMA_NEW] == self.IMA_NEW_SELL:
            #             #     if results[self.ACUMULADO_ABS] >= active.difference:
            #             #         results[self.CURRENT_ACTION] = self.ACTION_SELL
            #             #         alert = True
            #             #         isSell = True
            #             #         self.currentTupla['START'] = results[self.VALUE]
            ...
            #             #         self.prepararYenviarMensaje(results, active, data, True)
            #     # elif results[self.FLUJO] == self.FLUJO_SUBE:
            #     #     if results[self.INDICATOR_TENDENCE] == self.INDICATOR_T_DOWN:
            #     #         if results[self.ACTION_MIN_DIST] >= difference_optimized:
            #     #             results[self.CURRENT_ACTION] = self.ACTION_SELL
            #     #             alert = True
            #     #             isSell = True
            #     #             self.currentTupla['START'] = results[self.VALUE]

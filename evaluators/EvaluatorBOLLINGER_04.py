...
class EvaluatorBOLLINGER_04(EvaluatorBase,AperturaBase,CierreBase):
                ...
                if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                        ...
                        if results[Constants.PREVIOUS_DIST] <= (self.multiplicatorPREVIUS):
                            ...
                            print(f" INICIO MERCADO CHANGE_DOWN_SELL01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                ...
                if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                        ...
                        if results[Constants.PREVIOUS_DIST] <= (self.multiplicatorPREVIUS):
                            ...
                            print(f" INICIO MERCADO CHANGE_UP_BUY01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                    ...
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                        ...
                        if self.enableControlOpen:
                            if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(f" INICIO MERCADO CHANGE_UNDEF_UP_BUY01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                ...
                if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                        ...
                        if self.enableControlOpen:
                            if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(f" INICIO MERCADO SELLX5 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                ...
                if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                        ...
                        if self.enableControlOpen:
                            if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(f" INICIO MERCADO BUY4 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                ...
                if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                        ...
                        if self.enableControlOpen:
                            if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(f" INICIO MERCADO SELLXUNDEF02 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

                        else:
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                            print(f" INICIO MERCADO SELLUNDEF02 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                    ...
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized or results[
                        ...
                        if self.enableControlOpen:
                            if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(f" INICIO MERCADO BUYUNDEF02 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        if results[Constants.CLOSE_NXT_DOWN]==1:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                ...
                print(
                    f" INICIO MERCADO DOWN_BLG_SELL acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                ...
                if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                    if results[Constants.INDICATOR_EMA] == Constants.INDICATOR_EMA_SELL:
                        if results[Constants.IND_BLG_LOWER_DST]<self.blgLowerDist:
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                            print(
                                f" INICIO MERCADO DOWN_BLG_NXTUP_SELL02 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

                        else:
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(
                                f" INICIO MERCADO DOWN_BLG_NXTUP_BUY_02 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                    ...
                    if results[Constants.INDICATOR_EMA] == Constants.INDICATOR_EMA_SELL:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        print(
                            f" INICIO MERCADO DOWN_BLG_NXTUP_BUY_01 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
        ...
        if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) ==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                ...
                if results[Constants.IND_BLG] == Constants.IND_BLG_MED_BUY:
                    ...
                    if results[Constants.ACUMULADO] < 0:
                        if results[Constants.MEDSTDDIFF] < 0:
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                            print(
                                f" INICIO MERCADO DOWN_DOWN_SELL01 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                    ...
                    if results[Constants.IND_BLG_LOWER_DST] < self.blgLowerDist:
                        if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                            ...
                            if results[Constants.ACUMULADO] < 0:
                                if results[Constants.MEDSTDDIFF] < 0:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                    print(f" INICIO MERCADO DOWN_DOWN_SELL02 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

                            else:
                                if results[Constants.MEDSTDDIFF] >0:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    print(
                                        f" INICIO MERCADO DOWN_DOWN_SELLBUY03 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
            ...
            #                 results[Constants.NEW_ACTION] = Constants.ACTION_BUY
            #                 print(
            #                     f" INICIO MERCADO DOWN_DOWN_BUY_01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
    ...
    def evaluarAperturaPREDOWN(self, results, activeParam, flujo_count):
        ...
        if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
            ...
            if self.enableControlOpen:
                if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    print(f" INICIO MERCADO PREDOWN_DOWN_SELL01 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

            else:
                if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) ==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    print(f" INICIO MERCADO SELL PREDOWN_DOWN_SELL03 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
    ...
    def evaluarAperturaPREUP(self, results, activeParam, flujo_count):
        ...
        if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
            if self.enableControlOpen:
                if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                    print(f" INICIO MERCADO PREUP_UP_BUY01 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

            else:
                if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) ==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                    print(f" INICIO MERCADO PREUP_UP_BUY02 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                    if results[Constants.INDICATOR_EMA]==Constants.INDICATOR_EMA_BUY:
                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                        print(
                            f" INICIO MERCADO UP_BLG_NXTUP_SELL01 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                    ...
                    if results[Constants.INDICATOR_EMA]==Constants.INDICATOR_EMA_SELL:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        print(
                            f" INICIO MERCADO UP_BLG_NXTUP_BUY_01 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
        ...
        if results[Constants.CLOSE_NXT_DOWN]==1:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                    ...
                    results[Constants.CLOSE_NXT_DOWN] =0
                    print(
                        f" INICIO MERCADO UP_BLG_SELL acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                    ...
                    #         results[Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP)):
                        ...
                        results[Constants.CLOSE_NXT_DOWN] = 0
                        print(
                            f" INICIO MERCADO UP_BLG_SELL2 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
            ...
            if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_UP:
                ...
                #             results[Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP)):
                       if results[Constants.ACUMULADO] > 0:
                           if results[Constants.MEDSTDDIFF] > 0:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(f" INICIO MERCADO UP_UP_BUY01 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                           ...
                           #     results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                           #     print(
                           #         f" INICIO MERCADO UP_UP_SELL01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                else:
                    ##ESTAMOS SOBRE LA MEDIA
                        ...
                        if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                            ...
                            if results[Constants.ACUMULADO] > 0:
                                if results[Constants.MEDSTDDIFF] > 0:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    print(
                                        f" INICIO MERCADO UP_UP_BUY02 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

                            else:
                                if results[Constants.MEDSTDDIFF] < 0:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                    print(
                                        f" INICIO MERCADO UP_UP_SELL01 acum: {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]} \tWEEK_DIR {results[Constants.WEEK_DIR]} angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

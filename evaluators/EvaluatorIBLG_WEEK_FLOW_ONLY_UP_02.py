...
class EvaluatorIBLG_WEEK_FLOW_ONLY_UP_02(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #se añade cierre mas permisivo
        self.name = "EvaluatorIBLG_WEEK_FLOW_ONLY_UP_02"
        # self.name = "EvaluatorIBLG_WEEK_FLOW_ONLY_UP_01"
        # self.name = "EvaluatorIBLG_WEEK_TOP_ONLY_UP_02"
        # self.name = "EvaluatorIBLG_WEEK_TOP_ONLY_UP_01"
    ...
    def evaluarApertura(self, results, activeParam):
        ...
        elif "DOWN" in results[Constants.WEEK_DIR_FLOW]:
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        if results[Constants.CLOSE_NXT_UP] == 1:
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                ...
                # if results[Constants.WEEK_DIR_BOT_DST]<= self.week_minflow_bot_dst:
                #     results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                #     name = "DOWN_SELL_0"
                #     nameCount += 1
                #     print(
                #         f" INICIO MERCADO {name} {results[Constants.DATE].values[0]} \tACUMULADO_ABS: {results[Constants.ACUMULADO_ABS]} \tMEDSTDDIFF {results[Constants.MEDSTDDIFF]} \tINDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} \tDIRECTION {results[Constants.DIRECTION]} \tINDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                #     return
                if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                    ...
                    if results[Constants.IND_BLG_LOWER_DST_PERCENT] < self.blgDistPercent:
                        if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                            ...
                            if results[Constants.ANGLE] >= self.angleUp:

                                nada = ""
                                # if 'DOWN' not in results[Constants.WEEK_DIR_FLOW]:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                name = "DOWN_SELL_1"
                                nameCount+=1
#                                 print(
                    ...
                    if results[Constants.IND_BLG_LOWER_DST_PERCENT] < self.blgClosePercent:
                        if 'DOWN' not in results[Constants.WEEK_DIR_FLOW]:
                            if results[Constants.ANGLE] >= self.angleUp:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
#                                 print(
                                    f" INICIO MERCADO DOWN_LOWER_DST_01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]}  direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                    ...
                    #         results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    #         print(
                    #             f" INICIO MERCADO DOWN DOWN_SELLDST_01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
...
#                     print(
                ...
                if results[Constants.IND_BLG_UPPER_DST_PERCENT] >= self.blgDistPercent or results[Constants.IND_BLG_UPPER_DST_PERCENT]==0:
                    # if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                        ...
                        #         Constants.ANGLE] == 0:
                        # if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP:
                            if results[Constants.WEEK_DIR_TOP_DST] >=  self.blgDistPercent:
                                if results[Constants.ANGLE] > self.angleUp:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    name = "UP_BUY_2"
                                    nameCount += 1
#                                     print(
                                        f" INICIO MERCADO {name} {results[Constants.DATE].values[0]} \tACUMULADO_ABS: {results[Constants.ACUMULADO_ABS]} \tMEDSTDDIFF {results[Constants.MEDSTDDIFF]} \tINDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} \tDIRECTION {results[Constants.DIRECTION]} \tINDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                            elif results[Constants.WEEK_DIR_TOP_DST] <0:
                                nada = ""
                                ##esta subiendo
                        ...
                        if (results[Constants.ANGLE] > 0 and abs(results[Constants.ANGLE]) > self.angleUp) or results[
                            ...
                            if results[Constants.ANGLE] > self.angleUp:
                                # if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
#                                     print(
                                                f" INICIO MERCADO UP_UP_BUY_UPPPER_01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} INDICATOR_EMA: {results[Constants.INDICATOR_EMA]}   angle: {results[Constants.ANGLE]}   ANGLE_EMA: {results[Constants.ANGLE_EMA]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

...
class EvaluatorMEDMOMENT_CLEAN_Optimiz07_PERCENT(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorMEDMOMENT_CLEAN_Optimiz07_PERCENT"
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        if self.enableControlOpen:
            ...
            if abs(results[Constants.PREVIOUS_DIST]) <= results[Constants.PREVIOUS_DIST] and results[Constants.FLUJO_COUNT] <=self.previusOpenFlowCount:
                if results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN or results[
                    Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_UP_CHANGE:
                    if results[Constants.IND_REL_PERCENT_STD] == 0 or results[
                        Constants.IND_REL_PERCENT_STD] > self.percentMinDistance:

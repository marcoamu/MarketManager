...
class EvaluatorIBLG_ANGLE_FLOW_LONG_04(EvaluatorBase,AperturaBase,CierreBase):

    #tiene EMA MIN DST

    def __init__(self):
        #se añade blg_mid_distance closenxt_middle
        self.name = "EvaluatorIBLG_ANGLE_FLOW_LONG_04"
        # self.name = "EvaluatorIBLG_ANGLE_FLOW_LONG_02"
        # self.name = "EvaluatorIBLG_ANGLE_FLOW_LONG_01"
...
#                 print(f"FUERA DE HORARIO")
    ...
    def evaluarAperturaBLG(self, results, activeParam):
        ...
        # valores
        flujo_count = results[Constants.FLUJO_COUNT]
        if "BUY" in results[Constants.IND_BLG]:
            self.evaluarAperturaUP(results, activeParam, flujo_count)
        elif "SELL" in results[Constants.IND_BLG]:
            self.evaluarAperturaDOWN(results, activeParam, flujo_count)
        else:
            nada=""
            self.evaluarAperturaCHANGE(results, activeParam, flujo_count)
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        res, num = self.evaluateDownOpen_03(results,activeParam)
        if res:
            ...
            #revisar inversa
            if num < 0:
                #inversa reviar si puede abrir
                res, num = self.evaluateUpOpen_10(results, activeParam)
                if res:
                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                    self.printInicioLog(f"SELL_INV_BUY_{num}", results, activeParam)
                    return
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        if res:
            ...
            if num < 0:
                ...
                if res:
                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    self.printInicioLog(f"BUY_INV_SELL_{num}", results, activeParam)
                    return

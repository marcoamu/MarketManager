...
class EvaluatorEMA_IA_ONLY_UP_01(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorEMA_IA_ONLY_UP_01"
...
#             print(f"ESTAMOS A LA ESPERA DE INDICADORES BUENOS")
    ...
    def evaluarAperturaEMA(self, results, activeParam):
        ...
        # ================================
        ema_angle = self.safe_float(results.get(Constants.ANGLE_EMA))
        angle = self.safe_float(results.get(Constants.ANGLE))
        trend_intensity = results[Constants.MARKET_TREND_INTENSITY]


        # if angle is not None:
        #     if angle < float(activeParam.minEmaAngle):
        #         # print("paso 1  angle" + str(ema_angle))
        #         return  # mercado plano → NO operar

        # if float(trend_intensity) < float(activeParam.minTrendIntensity):
        ...
        ema_distance = abs(results[Constants.CURRVAL_EMA_DST])

        if float(ema_distance) < float(activeParam.minEmaDistance):
            # print("paso 2")
        ...
        # 🚀 4. DECISIÓN FINAL
        # ================================
        ema_signal = results[Constants.INDICATOR_EMA]

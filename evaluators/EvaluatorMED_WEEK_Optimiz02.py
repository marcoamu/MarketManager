...
class EvaluatorMED_WEEK_Optimiz02(EvaluatorBase, AperturaBase, CierreBase):

    def __init__(self):
        self.name = 'EvaluatorMED_WEEK_Optimiz02'
            ...
            if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP or results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UNDEF:
                if self.enableControlOpen:
                    ...
                    if difOpenHours <= 10:
                        ...
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        pass
            elif results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_DOWN or results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UNDEF:
                nada = ''
                if results[Constants.PREVIOUS_DIST] <= difference_optimized:
                    if results[Constants.ACUMULADO_ABS] >= difference_optimized * self.multiplicadorOpen:
                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                        pass
        elif results[Constants.FLUJO] == Constants.FLUJO_BAJA:

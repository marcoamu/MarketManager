...
class EvaluatorIBLG_LONG_ONLY_UP_05(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #se añaden mejoras apertura
    ...
    def evaluarFlujoBUY_LONG(self, results, activeParam):
        ...
        if results[Constants.CLOSE_NXT_DOWN] == 1:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                ...
                #
                print(
                        f" CERRAMOS MERCADO CLOSE_NXT:DOWN acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")

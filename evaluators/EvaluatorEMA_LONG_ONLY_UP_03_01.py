...
class EvaluatorEMA_LONG_ONLY_UP_03_01(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorEMA_LONG_ONLY_UP_03_01"
        # self.name = "EvaluatorEMA_LONG_ONLY_UP_01"
...
#             print(f"ESTAMOS A LA ESPERA DE INDICADORES BUENOS")
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateDownOpen_EMA_OUP_01(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateUpOpen_EMA_OUP_02(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
    ...
    def evaluarFlujoBUY(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoBUY", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_BUY_EMA_OUP_01(results, activeParam, self.closeAcumValue, self.closeDifference)

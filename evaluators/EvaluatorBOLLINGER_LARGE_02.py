...
class EvaluatorBOLLINGER_LARGE_02(EvaluatorBase, AperturaBase, CierreBase):

    def __init__(self):
        self.name = 'EvaluatorBOLLINGER_LARGE_02'
        ...
        if results[Constants.CLOSE_NXT_DOWN] == 1:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                    ...
                    return
        res, action, num = self.evaluateUpOpen_BLG_03(results, activeParam)
        if res:
            if 'BUY' in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY

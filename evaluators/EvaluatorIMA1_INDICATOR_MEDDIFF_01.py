...
class EvaluatorIMA1_INDICATOR_MEDDIFF_01(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorIMA1_INDICATOR_MEDDIFF_01"
        # self.name = "EvaluatorIMA1Optimiz_MEDDIFF_02"
    ...
    def evaluate(self, results, activeParam):
        ...
        self.maxAcumValue = activeParam.difference*3
        ...
        if results[Constants.CURRENT_ACTION] == Constants.ACTION_WAIT or results[
            ...
            self.evaluarAperturaINDICATOR(results, activeParam)
    ...
    def evaluarAperturaINDICATOR(self, results, activeParam):
        #
        # valores
        flujo_count = results[Constants.FLUJO_COUNT]

        if results[Constants.INDICATOR] == Constants.INDICATOR_BUY or results[Constants.INDICATOR] == Constants.INDICATOR_BUYX:
            self.evaluarAperturaUP(results, activeParam, flujo_count)
        elif results[Constants.INDICATOR] == Constants.INDICATOR_SELL or results[Constants.INDICATOR] == Constants.INDICATOR_SELLX:
            self.evaluarAperturaDOWN(results, activeParam, flujo_count)
        elif results[Constants.INDICATOR] == Constants.INDICATOR_WAIT:
            # nada = ""
            self.evaluarAperturaCHANGE(results, activeParam, flujo_count)
        else:
            print(f"ESTAMOS A LA ESPERA DE INDICADORES BUENOS")
    ...
    def evaluarAperturaCHANGE(self, results, activeParam, flujo_count):
        ...
        # if results[Constants.IMA1_DISTANCE] > self.ima1minDistance and results[
        #     Constants.IMA1_DISTANCE] < self.ima1maxDistance:
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                    ...
                    if difOpenHours <= 10:
                        ...
                        print(
                            f" INICIO MERCADO SELLWAIT00 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    else:
                        if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                            # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                                # if results[Constants.ACUMULADO_ABS] <= (difference_optimized * multiplicador):
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(f" INICIO MERCADO SELLWAIT01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                ...
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                    ...
                    if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                        # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                            ...
                            print(
                                f" INICIO MERCADO BUYWAIT01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                ...
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                    ...
                    if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                        # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                            # if results[Constants.ACUMULADO_ABS] <= (difference_optimized * multiplicador):
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(f" INICIO MERCADO BUYWAIT01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    ...
                    if difOpenHours <= 10:
                        ...
                        if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                            # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                                ...
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(
                                    f" INICIO MERCADO SELLWAIT01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                ...
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                    ...
                    if self.enableControlOpen:
                        if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                            print(f" INICIO MERCADO SELL_REVIEW01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    else:
                        if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                            # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(f" INICIO MERCADO SELL_REVIEW02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            elif results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                    Constants.ACTION_MIN_DIST] >= (difference_optimized * self.multiplicadorUP):
                    if self.enableControlOpen:
                        if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(f" INICIO MERCADO BUY_REVIEW01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    else:
                        if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                            # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(f" INICIO MERCADO BUY_REVIEW02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        # if results[Constants.IMA1_DISTANCE] > self.ima1minDistance and results[
        #     Constants.IMA1_DISTANCE] < self.ima1maxDistance:
        if abs(results[Constants.INDICATOR_DISTANCE]) > self.ima3minDistance:
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                ...
                #     Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UNDEF:
                    ...
                    if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                        ...
                        if self.enableControlOpen:
                            if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(f" INICIO MERCADO SELLX1 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        else:
                            difOpenHours = self.getDifTimeFromOpen(results)
                ...
                #             results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                #             print(f" INICIO MERCADO SELL WEEKUP 1 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                #
                ...
                if results[Constants.MARKET_TENDENCE] == Constants.MARKET_TENDENCE_UP:
                    if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                        ...
                        if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                            # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                print(f" INICIO MERCADO BUY INVERSA02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        #     print(f"dentro de indicator")
        if abs(results[Constants.INDICATOR_DISTANCE]) > self.ima3minDistance:
            # print(f"dentro de indicator1")
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE :
                # if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP or results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UNDEF:
                    if self.enableControlOpen:
                        # if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(f" INICIO MERCADO BUY1 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    else:
                        ...
                        if difOpenHours <=10:
                            ...
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                            print(f" INICIO MERCADO BUYOPEN1 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        else:
                            if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                                # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                ...
                #     nada = ""
                #     if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0:
                #         if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                #             results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                #             print(f" INICIO MERCADO BUYWEEKINV01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            # EVALUADORES A LA INVERSA VENTA
            elif results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                # if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_DOWN:
                    # if results[Constants.FLUJO_COUNT]>1:
                        if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                            ...
                            if self.enableControlOpen:
                                if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                    print(f" INICIO MERCADO SELL INVERSE2 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                            else:
                                if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF])==0:
                                    # if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
    ...
    def evaluarFlujoBUY(self, results, activeParam):
        ...
        if results[Constants.ACTION_ACUM] < 0:
            ...
            if abs(results[Constants.ACTION_ACUM]) >= self.closeLose:
                print(
            ...
            if results[Constants.ACTION_ACUM] >= self.closeAcumValue * 2:
                if results[Constants.ACUMULADO] <= 0:
                    if abs(results[Constants.INDICATOR_DISTANCE]) < self.ima3minDistance:
                        ...
                        if results[Constants.ACUMULADO_ABS] >= (self.closeAcumValue):
                            print(
                                f"CERRAMOS MERCADO BUY_ACTION_PROFIT_DIST02 {results[Constants.ACTION_ACUM]} profit:{self.closeAcumValue}")
                            results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                            return
            ...
            if abs(results[Constants.ACTION_ACUM]) >= self.closeAcumValue * 2:
                if results[Constants.ACUMULADO] > 0:
                    if abs(results[Constants.INDICATOR_DISTANCE]) < self.ima3minDistance:
                        ...
                        if results[Constants.ACUMULADO_ABS] >= (self.closeAcumValue):
                            print(
                                f"CERRAMOS MERCADO SELL_ACTION_PROFIT_DIST02 {results[Constants.ACTION_ACUM]} profit:{self.closeAcumValue}")
                            results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
                            return
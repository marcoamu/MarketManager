...
class EvaluatorMARKETTENDENCE_STDMED_MEDDIFF_01_ONLYUP(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        self.name = "EvaluatorMARKETTENDENCE_STDMED_MEDDIFF_01_ONLYUP"
        # self.name = "EvaluatorMARKETTENDENCE_STDMED_MEDDIFF_01"
        # self.name = "EvaluatorMARKETTENDENCE_STDMED_01"
    ...
    def evaluate(self, results, activeParam):
        ...
        self.multiplicadorPrevious = 2.2
        ...
        #para este caso duplicamos
        # self.closeAcumValue = self.closeAcumValue *2
...
#                             print(
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                ...
                #             results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                #             print(
                #                 f" INICIO MERCADO SELL_REVIEW01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                #     else:
                #         if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(
                #                 results[Constants.MEDSTDDIFF]) == 0:
                #             if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                #                 results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                #                 print(
                #                     f" INICIO MERCADO SELL_REVIEW02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            elif results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                    Constants.ACTION_MIN_DIST] >= (difference_optimized * self.multiplicadorUP):
                    if self.enableControlOpen:
                        if results[Constants.ACUMULADO_ABS] <= (difference_optimized * self.multiplicadorOpen):
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
#                             print(
                                f" INICIO MERCADO BUY_REVIEW01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                    else:
                        if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(
                                results[Constants.MEDSTDDIFF]) == 0:
                            if abs(results[Constants.PREVIOUS_DIST]) <= self.maxAcumValue:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
#                                 print(
                                    f" INICIO MERCADO BUY_REVIEW02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
...
#             print(f"difOpenHours {difOpenHours}")
            if difOpenHours <= 20:
                ...
                #         difference_optimized * self.multiplicadorOpen):
                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
#                     print(f" INICIO MERCADO DOWN_DOWN_SELL02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            else:
                ...
                #     Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_UP_CHANGE:
                    if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                        ...
                        if results[Constants.IND_PROB_FLOW] == Constants.DIR_DOWN:
...
#                                 print(f" INICIO MERCADO DOWN_DOWN_SELL01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                            ...
                            # if abs(results[Constants.PREVIOUS_DIST]) <= (
                            #         difference_optimized * self.multiplicadorPrevious):
                                if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) ==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
            ...
            #     Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN_CHANGE:
                if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                    ...
                    if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) ==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                        ...
                        #                 difference_optimized * self.multiplicadorPrevious):
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
#                             print(f" INICIO MERCADO DOWN_UP_SELL04 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
...
#                         print(f" INICIO MERCADO SELL PREDOWN_DOWN_SELL02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                ...
                if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
                    Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP):
                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
#                     print(f" INICIO MERCADO SELL PREDOWN_UP_SELL01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            # elif results[Constants.MARKET_TENDENCE] == Constants.MARKET_TENDENCE_UNDEF:
            #     if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
            #         Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP):
            #         results[Constants.NEW_ACTION] = Constants.ACTION_SELL
            #         print(f" INICIO MERCADO PREDOWN_UNDEF_SELL01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
...
#                         print(f" INICIO MERCADO PREUP_UP_BUY02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            ...
            #         Constants.ACTION_MAX_DIST] >= (difference_optimized * self.multiplicadorUP):
            #         results[Constants.NEW_ACTION] = Constants.ACTION_BUY
            #         print(f" INICIO MERCADO PREUP_DOWN_BUY01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            # elif results[Constants.MARKET_TENDENCE] == Constants.MARKET_TENDENCE_UNDEF:
            #     if results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or results[
            #         Constants.ACTION_MAX_DIST] >= (difference_optimized* self.multiplicadorUP):
            #         results[Constants.NEW_ACTION] = Constants.ACTION_BUY
            #         print(f" INICIO MERCADO PREUP_UNDEF_BUY01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
...
#         print(
        ...
        if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
            ...
            if difOpenHours <=20:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
#                 print(f" INICIO MERCADO UP_BUY02 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
            else:
                # if results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_UP or results[
                #     Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN_CHANGE:
                    if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                        ...
                        if results[Constants.IND_PROB_FLOW] == Constants.DIR_UP:
                            if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) ==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
#                                 print(f" INICIO MERCADO UP_BUY01 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
                        else:
                            # if abs(results[Constants.PREVIOUS_DIST]) <= (
                            #         difference_optimized * self.multiplicadorUP):
                                if (abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff or abs(results[Constants.MEDSTDDIFF]) ==0) and abs(results[Constants.MEDSTDDIFF]) < self.medstdmaxDiff:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
#                                     print(f" INICIO MERCADO UP_BUY03 acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")
        # elif results[Constants.FLUJO] == Constants.FLUJO_BAJA:
        #     # if results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN or results[
        #     #     Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_UP_CHANGE:
        #         if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
        ...
        #                         #     Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN_CHANGE:
        #                             results[Constants.NEW_ACTION] = Constants.ACTION_BUY
        #                             print(f" INICIO MERCADO DOWN_BUY04: acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} percentSTD: {results[Constants.IND_REL_PERCENT_STD]}")

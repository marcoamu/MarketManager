...
class EvaluatorIBLG_WEEK_FLOW_03(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        ...
        self.name = "EvaluatorIBLG_WEEK_FLOW_03"
        # self.name = "EvaluatorIBLG_WEEK_FLOW_02"
    ...
    def evaluate(self, results, activeParam):
        ...
        if results[Constants.CURRENT_ACTION] == Constants.ACTION_WAIT or results[
            ...
            if intime:
                # self.evaluarAperturaWEEKDIR(results, activeParam)
                self.evaluarAperturaWEEKFLOW(results, activeParam)
            else:
#                 print(f"FUERA DE HORARIO")
    ...
    def evaluarAperturaWEEKDIR(self, results, activeParam):
        #
        # valores
...
#             print(f"ESTAMOS A LA ESPERA DE INDICADORES BUENOS")
    ...
    def evaluarAperturaWEEKFLOW(self, results, activeParam):
        #
        # valores
        flujo_count = results[Constants.FLUJO_COUNT]
        ...
        if "UP" in results[Constants.WEEK_FLOW]:
            self.evaluarAperturaUP(results, activeParam, flujo_count)
        elif "DOWN" in results[Constants.WEEK_FLOW]:
            self.evaluarAperturaDOWN(results, activeParam, flujo_count)
        elif results[Constants.IND_BLG] == Constants.IND_BLG_MED_WAIT:
            nada=""
            self.evaluarAperturaCHANGE(results, activeParam, flujo_count)
...
#                             print(
                        ...
                        nada=""
                        if "DOWN" in results[Constants.WEEK_DIR_FLOW]:
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
#                             print(
                        ...
                        elif "UP" in results[Constants.WEEK_DIR_FLOW]:
                            results[Constants.NEW_ACTION] = Constants.ACTION_BUY
#                             print(

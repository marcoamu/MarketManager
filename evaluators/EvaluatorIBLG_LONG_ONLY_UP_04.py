...
class EvaluatorIBLG_LONG_ONLY_UP_04(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #se añade cierre mas permisivo
        self.name = "EvaluatorIBLG_LONG_ONLY_UP_04"
    ...
    def evaluarFlujoBUY_LONG(self, results, activeParam):
        if results[Constants.IND_BLG]== Constants.IND_BLG_MED_SELL:
            if results[Constants.ACTION_COUNT]>5:
                if results[Constants.ACTION_MAX_DIST] > self.closeDifference:
                    print(
        ...
        if results[Constants.ACTION_COUNT] > 5:
            if results[Constants.ACTION_ACUM] <= 0:
                ...
                if results[Constants.ACTION_ACUM] <=self.closeDifference:
                    print(

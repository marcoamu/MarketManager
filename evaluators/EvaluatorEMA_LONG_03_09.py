...
class EvaluatorEMA_LONG_03_09(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorEMA_LONG_03_09"
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateUpOpen_EMA_WEEK_NEW_FLOW_02(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
    ...
    def evaluarFlujoBUY(self, results, activeParam):
        ...
        self.printDifference("evaluarFlujoBUY", difference_optimized, self.closeAcumValue, self.closeDifference,
                             self.accumulate)
        self.control_BUY_WEEK_NEW_DIFF_02(results, activeParam, self.closeAcumValue, self.closeDifference)

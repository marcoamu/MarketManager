...
class EvaluatorEMA_IA_LONG_ONLY_UP_CROSS(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorEMA_IA_LONG_ONLY_UP_CROSS"
        # self.name = "EvaluatorEMA_LONG_02"
    ...
    def evaluarAperturaEMA_IA(self, results, activeParam):
        ...
        # ================================
        ema_angle = results.get(Constants.EMA10_ANGLE, 0)
        trend_intensity = results.get(Constants.MARKET_TREND_INTENSITY, 0)

        if abs(ema_angle) < activeParam.minEmaAngle:
            return  # mercado plano → NO operar

        if trend_intensity < activeParam.minTrendIntensity:
            return  # poco momentum → NO operar

        # ================================
        # 🚫 2. EVITAR ZONA MUERTA (EMA)
        # ================================
        ema_distance = abs(results.get(Constants.CURRVAL_EMA_DST, 0))

        if ema_distance < activeParam.minEmaDistance:
            return  # precio demasiado cerca de EMA → ruido

        # ================================
        # 📊 3. FILTRO BOLLINGER
        # ================================
        blg_lower = results.get(Constants.IND_BLG_LOWER_DST_PERCENT, 0)
        blg_upper = results.get(Constants.IND_BLG_UPPER_DST_PERCENT, 0)

        # ================================
        # 🚀 4. DECISIÓN FINAL
        # ================================
        ema_signal = results[Constants.INDICATOR_EMA50]

        # ✅ LONG
        if ema_signal == Constants.INDICATOR_EMA_BUY:
        ...
        # 🔻 SHORT
        elif ema_signal == Constants.INDICATOR_EMA_SELL:

            # Confirmación: cerca banda alta + tendencia negativa
            if blg_upper > activeParam.bollingerHighThreshold and ema_angle < 0:
                self.evaluarAperturaDOWN(results, activeParam, flujo_count)

        elif ema_signal == Constants.INDICATOR_EMA_WAIT:
            return
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        ema10_slope = float(results.get(Constants.EMA10_SLOPE))
        is_lateral = False
        if cross_count >= activeParam.cross_count  or  ema10_slope < 0.001:# or bandwidth < 0.01:
        # if cross_count >= activeParam.cross_count :# or bandwidth < 0.01:
            is_lateral = True

...
class EvaluatorEMA_LONG_03_04(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        self.name = "EvaluatorEMA_LONG_03_04"
    ...
    def evaluarAperturaDOWN(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateDownOpen_EMA_WEEK_02(results, activeParam)
        if res:
            if "BUY" in action:
                nada = ""
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        res, action, num = self.evaluateUpOpen_EMA_WEEK_04(results, activeParam)
        if res:
            if "BUY" in action:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY

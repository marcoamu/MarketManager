...
class EvaluatorETHUSD_ONLY_BUY_01(EvaluatorBase, AperturaBase, CierreBase):
    ...
    Estrategia ULTRA-AGRESIVA de solo compra para ETH/USD.
    NO abre posiciones SELL en ningún caso.

    ETH vs BTC — diferencias clave consideradas:
    ─────────────────────────────────────────────────
    · ETH tiene mayor volatilidad beta que BTC → umbrales más amplios
    · ETH reacciona más rápido a noticias de red/gas → Order Flow es crítico
    · Los rebotes V-shape son más frecuentes y violentos en ETH
    · RSI en ETH puede mantenerse en sobrecompra más tiempo → threshold mayor
    · El canal Bollinger en ETH es más ancho → blgLowerDistPercemt más amplio

    Indicadores utilizados (confluencia multi-señal):
    ─────────────────────────────────────────────────
    1.  EMA            → dirección principal de tendencia
    2.  Bollinger Bands → posicionamiento de precio en el canal (umbral 8%)
    3.  RSI            → momentum; sobreventa (<32) entrada, sobrecompra (>72) cierre
    ...
    ─────────────────────────────────────────────────
    A. EMA BUY + Week UP + RSI < 42 + Order Flow BUY → entrada inmediata
    B. RSI sobreventa (<32) + Bollinger inferior cerca + Order Flow BUY
    ...
    E. Rebote V-shape: RSI<25 + BLG lower perforado + Order Flow BUY fuerte
       (ETH tiene rebotes V más violentos que BTC)
    F. MACD bullish crossover + WEEK UP + flujo SUBE

    Reglas de cierre:
    ─────────────────────────────────────────────────
    1. RSI sobrecompra (>72) → salida agresiva con ganancia
    ...
    5. Ángulo bajista fuerte (contador >= 3) → salida preventiva
    6. Stop-loss agresivo adaptado a la volatilidad ETH
    7. Cierre estándar heredado (EMA + WEEK)

    Versión: ETHUSD_ONLY_BUY_01
    """
...
#         print(f"[ETHUSD_ONLY_BUY] ⚠️  POSICIÓN SELL DETECTADA — cerrando inmediatamente")
    ...
    def _rsi_oversold(self, results, threshold=32.0):
        """RSI en zona de sobreventa (< threshold). ETH: 32 vs BTC 30."""
        try:
            rsi = float(results.get(Constants.RSI, 50))
            return rsi < threshold
        except Exception:
    ...
    def _rsi_extreme_oversold(self, results, threshold=26.0):
        """RSI en zona de sobreventa extrema. ETH: 26 vs BTC 25."""
        try:
            rsi = float(results.get(Constants.RSI, 50))
            return rsi < threshold
        except Exception:
    ...
    def _strong_confluence_buy(self, results, activeParam):
        """
        Confluencia triple para compra en ETH/USD:

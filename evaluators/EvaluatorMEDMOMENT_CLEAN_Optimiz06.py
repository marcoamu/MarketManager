        ...
        if results[Constants.INDICATOR_MED_MOMENT_VALUE] > 0:
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_BAJA:
                if results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN or results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_UP_CHANGE:
                    if results[Constants.IND_PROB_FLOW] == Constants.DIR_PRE_DOWN or results[Constants.IND_PROB_FLOW] == Constants.DIR_DOWN:
                        if results[Constants.IND_REL_PERCENT_STD] == 0 or results[Constants.IND_REL_PERCENT_STD] > self.percentMinDistance:
        ...
        if self.enableControlOpen:
            ...
            if difOpenHours <= 10:
                if results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN or results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_UP_CHANGE:
                    results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    pass
            elif abs(results[Constants.PREVIOUS_DIST]) <= self.previusOpenDistance and results[Constants.FLUJO_COUNT] <= self.previusOpenFlowCount:
                if results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_DOWN or results[Constants.RELATIVE] == Constants.INDICATOR_RELATIVE_UP_CHANGE:
                    if results[Constants.IND_REL_PERCENT_STD] == 0 or results[Constants.IND_REL_PERCENT_STD] > self.percentMinDistance:
                        results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                        pass

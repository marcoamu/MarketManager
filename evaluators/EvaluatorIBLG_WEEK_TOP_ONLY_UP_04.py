...
class EvaluatorIBLG_WEEK_TOP_ONLY_UP_04(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #se añade cierre mas permisivo
        self.name = "EvaluatorIBLG_WEEK_TOP_ONLY_UP_04"
        # self.name = "EvaluatorIBLG_WEEK_TOP_ONLY_UP_03"
                ...
                if (results[Constants.ACUMULADO_ABS] >= (difference_optimized * self.multiplicadorUP) or
                    ...
                    if results[Constants.IND_BLG_LOWER_DST_PERCENT] < self.blgDistPercent:
                        ...
                        if results[Constants.INDICATOR_MED] == Constants.INDICATOR_TM_DOWN:
                            if results[Constants.WEEK_FLOW_MED] > 0:
                                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                name = "DOWN_SELL_1"
                                self.printInicioLog(name,results,activeParam)
                            else:
                                if abs(results[Constants.WEEK_FLOW_MED]) < self.week_med_min:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    name = "DOWN_SELL_WEEK_NEG_2"
                                    nameCount += 1
                                    self.printInicioLog(name, results, activeParam)
                                else:
                                    if results[Constants.WEEK_DIR_BOT_DST] < 5:
                                        results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                        name = "DOWN_NXT_WEEK_BOT_01"
                                        nameCount += 1
                                        self.printInicioLog(name, results, activeParam)
                ...
                if results[Constants.IND_BLG_LOWER_DST_PERCENT] >= self.blgDistPercent or results[Constants.IND_BLG_LOWER_DST_PERCENT] ==0:
                    if abs(results[Constants.MEDSTDDIFF]) > self.medstdminDiff:
                        if results[Constants.ANGLE_IMA1]> 0:
                            if results[Constants.ANGLE_IMA1_COUNTER] >=2:
                                if results[Constants.ANGLE_IMA1] > self.ima1minDistance:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    name = "DOWN_IMA1_01"
                                    self.printInicioLog(name, results, activeParam)
                    ...
                    if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP:
                        if results[Constants.IND_BLG_LOWER_DST_PERCENT] < self.blgClosePercent:
                            ...
                            #     if abs(results[Constants.ANGLE]) < self.minAngleIma1:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    self.printInicioLog("DOWN_LOWER_DST_01", results, activeParam)
                    ...
                    if results[Constants.IND_BLG_UPPER_DST_PERCENT] < 0:
                        ...
                        if (results[Constants.ANGLE] > 0 and abs(results[Constants.ANGLE]) > self.angleUp) or results[
                            ...
                            if results[Constants.ANGLE_EMA]>0:
                                if results[Constants.WEEK_FLOW] == Constants.WEEK_FLOW_UP:
                                    results[Constants.NEW_ACTION] = Constants.ACTION_BUY
                                    self.printInicioLog("UP_UP_BUY_UPPPER_01", results, activeParam)
                                #abrimos una venta

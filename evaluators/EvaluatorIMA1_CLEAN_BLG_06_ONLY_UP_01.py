...
class EvaluatorIMA1_CLEAN_BLG_06_ONLY_UP_01(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #SE AÑADEN CONTROLES WEEK PARA ABRIR TEMPRANO
        self.name = "EvaluatorIMA1_CLEAN_BLG_06_ONLY_UP_01"
        # self.name = "EvaluatorIMA1_CLEAN_BLG_06"
        # self.name = "EvaluatorIMA1_CLEAN_BLG_05"
    ...
    def evaluarFlujoBUY(self, results, activeParam):
        ...
        #CERAMMOS SI LLEGA A TOP BLG
        if results[Constants.IND_BLG_UPPER_DST_PERCENT] <0 or results[Constants.IND_BLG_UPPER_DST_PERCENT] < 5:
            print(
                f"CERRAMOS MERCADO BUY action_acum: {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_BLG_UPPER_01 profit:{results[Constants.ACTION_ACUM]} ")
            results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
            return
    ...
    def evaluarFlujoSELL(self, results, activeParam):
        ...
        # CERAMMOS SI LLEGA A TOP BLG
        if results[Constants.IND_BLG_LOWER_DST_PERCENT] < 0 or results[Constants.IND_BLG_LOWER_DST_PERCENT] < 5:
            print(
                f"CERRAMOS MERCADO SELL action_acum: {results[Constants.ACTION_ACUM]} {results[Constants.DATE].values[0]} BUY_BLG_LOWER_01 profit:{results[Constants.ACTION_ACUM]} ")
            results[Constants.NEW_ACTION] = Constants.ACTION_CLOSE
            return

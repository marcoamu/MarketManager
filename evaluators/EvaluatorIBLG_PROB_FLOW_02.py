...
class EvaluatorIBLG_PROB_FLOW_02(EvaluatorBase,AperturaBase,CierreBase):
    ...
    def __init__(self):
        ...
        # self.name = "EvaluatorIBLG_PROB_FLOW_01"
        ...
        if results[Constants.ACUMULADO_ABS] >= (difference_optimized) or results[
            ...
            if self.enableControlOpen:
                ...
                if results[Constants.FLUJO]==Constants.FLUJO_BAJA:
                    if abs(results[Constants.EMA_DST]) >= self.emaMinDst:
                        if results[Constants.IND_BLG_LOWER_DST_PERCENT] >= self.blgDistPercent or results[Constants.IND_BLG_LOWER_DST_PERCENT] ==0:
                            ...
                            if results[Constants.ANGLE]> 0:
                                #ESTA SUBIENDO pero estamos de bajada temporal cerramos proxmimo MIDDLE
                                results[Constants.CLOSE_NXT_MIDDLE] = 1
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                    ...
                    if results[Constants.ANGLE_IMA1] <0 and results[Constants.ANGLE_IMA1_COUNTER] >=3:
                        #ESTA BAJANDO

                        if results[Constants.IND_BLG_LOWER_DST_PERCENT] >= self.blgDistPercent or results[
                            Constants.IND_BLG_LOWER_DST_PERCENT] == 0:
                                results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                                print(
                                    f" INICIO MERCADO DOWN_DOWN_IMACOUNTER_01 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]}  angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")
                            ...
                            results[Constants.NEW_ACTION] = Constants.ACTION_SELL
                            print(
                                f" INICIO MERCADO DOWN_DOWN_IMACOUNTER_02 {results[Constants.DATE].values[0]} Acum: {results[Constants.ACUMULADO_ABS]} medddiff {results[Constants.MEDSTDDIFF]} \tEMA_DST {abs(results[Constants.EMA_DST])}  \tWEEK_FLOW_DIFF {results[Constants.WEEK_FLOW_DIFF]}  angle: {results[Constants.ANGLE]} direction {results[Constants.DIRECTION]} INDICATOR_MED_MOMENT {results[Constants.INDICATOR_MED_MOMENT]}")

                        elif results[Constants.IND_BLG_LOWER_DST_PERCENT] < self.blgDistPercent or results[
                            Constants.IND_BLG_LOWER_DST_PERCENT] == 0:
                            nada = ""

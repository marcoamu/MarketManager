...
class EvaluatorIBLG_ANGLE_ONLY_UP_05_test08(EvaluatorBase,AperturaBase,CierreBase):

    def __init__(self):
        #se añade cierre mas permisivo
        #se añade ontrol de apertura nxt up usando angleFlow
        self.name = "EvaluatorIBLG_ANGLE_ONLY_UP_05_test08"
    ...
    def evaluate(self, results, activeParam):
        ...
        if results[Constants.CURRENT_ACTION] == Constants.ACTION_WAIT or results[
            ...
            self.evaluarAperturaANGLE_FLOW(results, activeParam)
            ...
            if results[Constants.FLUJO] == Constants.FLUJO_SUBE:
                ...
                if res:
                    if results[Constants.HOURLY_ANGLE_MED] > 0:
                        if results[Constants.ANGLE_COUNTER]<=10:
    ...
    def evaluarAperturaUP(self, results, activeParam, flujo_count):
        ...
        res, option = self.evaluateUpOpen_08_IMP01(results, activeParam)
        if res:
            # if results[Constants.HOURLY_ANGLE_MED] > 0:
                results[Constants.NEW_ACTION] = Constants.ACTION_BUY
    ...
    def evaluarFlujoBUY_LONG(self, results, activeParam):
        ...
        self.control_BUY_LONG_ONLYUP_08(results,activeParam,self.closeAcumValue,self.closeDifference,self.accumulate)
